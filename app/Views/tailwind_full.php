...
<nav class="ml-10" aria-label="<?= lang('Pager.pageNavigation') ?>">
  ...
  <?php foreach ($pager->links() as $link): ?>
        <li <?= $link['active'] ? 'class="active"' : '' ?> >
            <a href="<?= $link['uri'] ?>" class="relative block rounded bg-purple-100 px-5 py-3 text-lg font-medium text-purple-700 transition duration-300 focus:outline-none dark:bg-slate-900 dark:text-purple-500"
              ><?= $link['title'] ?>
              <span
